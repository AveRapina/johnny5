...
bool ms5611Init()
{
	...
	ms5611_i2chandle = i2cOpen(1, MS5611_ADDR_CSB_LOW, 0);	//打开I2C

	if (ms5611_i2chandle >= 0)	//若I2C打开成功
	{
...
void ms5611StartConversion(uint8_t command)
{
  ...
  i2cdevWriteByte(ms5611_i2chandle, I2CDEV_NO_MEM_ADDR, command);
}
...
int32_t ms5611GetConversion(uint8_t command)
{
  ...
  // start read sequence
  i2cdevWriteByte(ms5611_i2chandle, I2CDEV_NO_MEM_ADDR, 0);
  // Read conversion
  i2cdevRead(ms5611_i2chandle, I2CDEV_NO_MEM_ADDR, MS5611_D1D2_SIZE, buffer);
...
bool ms5611ReadPROM()
{
  ...
  {
    // start read sequence
    status = i2cdevWriteByte(ms5611_i2chandle, I2CDEV_NO_MEM_ADDR,
                             MS5611_PROM_BASE_ADDR + (i * MS5611_PROM_REG_SIZE));
    // Read conversion
    if (status)
    {
      status = i2cdevRead(ms5611_i2chandle, I2CDEV_NO_MEM_ADDR, MS5611_PROM_REG_SIZE, buffer);
...
void ms5611Reset()
{
  i2cdevWriteByte(ms5611_i2chandle, I2CDEV_NO_MEM_ADDR, MS5611_RESET);
}
